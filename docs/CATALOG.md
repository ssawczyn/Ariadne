...
| 6 | Command palette selections not announced | Core UI | Needs investigation | Medium | The fix for #1 turned out to be generic and already covers two different trigger contexts (wikilinks, tags) with no extra code, which raises confidence the command palette shares the same underlying popup component — but that's still an inference, not a test. Worth trying `ariadne-autocomplete` against it directly. |
