...
| 6 | Command palette selections not announced | Core UI | Needs investigation | Medium | Tested `ariadne-autocomplete` against it directly — no effect. Likely a different underlying component: the wikilink/tag suggesters use `EditorSuggest` (inline, triggered while typing in the editor), while the command palette is probably `SuggestModal`/`FuzzySuggestModal` (a full modal dialog) — visually similar but probably different markup, so the `.suggestion-container` detection in `ariadne-autocomplete` never activates. Focus staying on the search input while arrowing is expected combobox behavior, not itself the bug — the missing piece is whatever should be signaling the highlight change, which was never wired up because detection didn't fire. Needs its own investigation into the modal's actual DOM structure before scoping a fix. |
