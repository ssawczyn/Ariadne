...
|---|-------|------|-------|------------|-------|
| 1 | `[[` wikilink autocomplete: no ARIA announcement that suggestions are available; no keyboard navigation semantics exposed to AT | Editor — suggestion popup | Plugin-fixable | High | Obsidian's `EditorSuggest` popup appears to be a bespoke UI, not built on CM6's `@codemirror/autocomplete` (which already implements the accessible ARIA combobox pattern). First planned fix. |
