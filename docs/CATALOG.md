...
|---|-------|------|-------|------------|-------|
| 1 | `[[` wikilink autocomplete: no ARIA announcement that suggestions are available; no keyboard navigation semantics exposed to AT | Editor — suggestion popup | **Fixed** | High | Fixed in [ariadne-autocomplete](https://github.com/ssawczyn/ariadne-autocomplete). Confirmed live on macOS/VoiceOver: availability announcement, arrow-key selection reading, accurate count while narrowing the query, and clean Escape dismissal all work. Obsidian's popup is a bespoke `EditorSuggest` UI, not built on CM6's `@codemirror/autocomplete`, which is why none of this worked natively. Not yet tested on Windows/NVDA or other AT+OS combinations — help wanted. |
...
| 6 | Command palette selections not announced | Core UI | Needs investigation | Low | May share root cause with #1 if the command palette uses the same underlying `Suggest` popup component — would make it a "fix once, benefits both" case. |
